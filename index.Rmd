...
lef <- data_frame(
  ...
  mutate(epsilon = rnorm(n(), 0, sigma_e), 
         m_c = int + b_site + b_plot + b_tree + b_core + b_depth + epsilon)  %>% 
  filter(depth != "D2")  # removing 2-4 chunk
...
bzn <- data_frame(
  ...
  depth = paste0("D", rep(1:n_depth, times = n_obs / n_depth)), 
  species = "DF", 
  elevation = ifelse(site %in% c("S1", "S2"), "H", "L"), 
  aspect = ifelse(site %in% c("S1", "S3"), "N", "S"), 
  ...
  mutate(epsilon = rnorm(n(), 0, sigma_e), 
         m_c = int + b_site + b_plot + b_tree + b_core + b_depth + epsilon)  %>% 
  filter(species == "DF", ## selecting only Douglas Firs
         depth != "D2")   ## removing 2-4 chunk
